...
contract PollAdmin is PollView, PollHelpers, IPollAdmin, IPollValidator {
    ...
    @return an array of all respondets answers as an array of uint corresponding to the index of the choices selected by the respondent
     */
    ...
    @dev if the poll is rejected, the DAO will refund the owner less the validators fees
...
//A EFFACER
    ...
    function _refund() private onlyOwner() {
        ...
        require(balance <= address(this).balance, 'Not enough funds');
        // require(pollStatus == PollStatus.PollingEnded, 'Poll already distributed');
